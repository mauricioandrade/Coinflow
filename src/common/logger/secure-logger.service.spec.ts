...
function spyOnParentMethod(
  ...
  return jest.spyOn(ConsoleLogger.prototype, methodName).mockImplementation(() => {});
}
...
describe('SecureLogger', () => {
  ...
  describe('sensitive key masking', () => {
    ...
    it('should redact the "encryptedCredentials" key', () => {
      const spy = spyOnParentMethod('log');
      logger.log({ encryptedCredentials: 'aabbcc:ddeeff:001122', institutionId: 'itau' });
      expect(spy).toHaveBeenCalledWith(
  ...
  describe('value-pattern masking', () => {
    ...
    it('should redact long hex strings (>= 32 chars) — likely keys or ciphertext', () => {
      const spy = spyOnParentMethod('log');
      const hexKey = 'a1b2c3d4e5f60718293a4b5c6d7e8f90a1b2c3d4e5f60718293a4b5c6d7e8f9';
  ...
  describe('deep sanitization', () => {
    ...
    it('should sanitize objects inside arrays', () => {
      const spy = spyOnParentMethod('log');
      logger.log({ users: [{ name: 'Alice', password: 'abc' }, { name: 'Bob', password: 'xyz' }] });
      const received = spy.mock.calls[0][0] as { users: Array<{ name: string; password: string }> };
