...
import { TestJwtModule, TEST_JWT_SECRET, TEST_USER_PAYLOAD } from '../helpers/test-jwt.helper';
...
describe('JwtAuthGuard (E2E)', () => {
  ...
  describe('Protected route without JWT', () => {
    ...
    it('should return 401 when token is signed with a wrong secret', () => {
      const fakeToken = jwtService.sign(TEST_USER_PAYLOAD, { secret: 'wrong-secret-entirely' });
      return request(app.getHttpServer())
        .get('/api/v1/auth-test/protected')
    ...
    it('should return 401 when token has expired', () => {
      const expiredToken = jwtService.sign(TEST_USER_PAYLOAD, { expiresIn: '-1s' });
      return request(app.getHttpServer())
        .get('/api/v1/auth-test/protected')
